...
all : damo.native printbig.o symbol.o
...
clean :
	ocamlbuild -clean
	rm -rf testall.log *.diff damo scanner.ml parser.ml parser.mli
	rm -rf printbig
	rm -rf *.dml *.cmx *.cmi *.cmo *.cmx *.o *.s *.ll *.out *.exe

# More detailed: build using ocamlc/ocamlopt + ocamlfind to locate LLVM

OBJS = ast.cmx codegen.cmx parser.cmx scanner.cmx semant.cmx damo.cmx

damo : $(OBJS)
	ocamlfind ocamlopt -linkpkg -package llvm -package llvm.analysis $(OBJS) -o damo
...
printbig : printbig.c
	cc -o printbig -DBUILD_TEST printbig.c -lm

symbol : symbol.c
	cc -o symbol -DBUILD_TEST symbol.c -lm
...
codegen.cmo : ast.cmo
codegen.cmx : ast.cmx
damo.cmo : semant.cmo scanner.cmo parser.cmi codegen.cmo ast.cmo
damo.cmx : semant.cmx scanner.cmx parser.cmx codegen.cmx ast.cmx
...
TARFILES = ast.ml codegen.ml Makefile damo.ml parser.mly README scanner.mll \
