...
let translate (program_unit_list) =
  ...
  *)
  ...
  let rec combine_prog_units prog_stmts main_stmts gvars func_unit=
     (match prog_stmts with
  ...
  topstmts, gvars, functions = combine_prog_units program_unit_list [] [] [] in
  ...
  *)
  let main_function = {
    ...
    A.fname = "main";
    A.formals = [];
    A.locals = gvars; (*main_binds;*)
    A.body = topstmts; (*filtered_main_stmts;*)
  } in
          ...
          A.Decl( t, s ) -> 
              (match t with
                 ...
                 | A.Num -> let init = L.const_float (ltype_of_typ t) 0.0 in
                    StringMap.add s (L.define_global s init the_module) m
                 | A.String -> let init = L.const_stringz (ltype_of_typ t) "" in
                    StringMap.add s (L.define_global s init the_module) m
    ...
    *)
    ...
    let rec expr builder = function
      ...
      | A.Binop (t, e1, op, e2) ->
        ...
        let e1', e2' = binop_type_check(t, e1, e2) in 
        ...
        *)
        ...
        then build exp funtion then cast back to int *)
            ...
            | A.Int -> (* Power fxn only defined in terms of floats*)
               let e1_cast = L.build_sitofp e1' num_t "cast" builder in
        ...
        *)
        ...
        let build_ops_with_types t = 
          match (t) with
            A.Int -> int_bop op
          | A.Num -> num_bop op
          | _,_ -> ignore( print_string "build op exception"); num_bop op
        in
        (build_ops_with_types t) e1' e2'
      ...
      | A.Assign (t, s, e) -> let e' = expr builder e in ignore( let (s_v, _) = (lookup s) in
